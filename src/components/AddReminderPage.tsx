...
export default function AddReminderPage({ cases, onAddEvent, onUpdateEvent, onBack, editingEvent, onAddDocument }: AddReminderPageProps) {
  ...
  const handleSaveReminder = () => {
    ...
    const extraDetails = [
      ...
      selectedCaseId ? `مرتبط با پرونده: ${cases.find(c => c.id === selectedCaseId)?.title || ""}` : "",
      `هشدارها: ${alerts.join(", ")}`,
      `تکرار: ${repeatSelected !== "بدون تکرار" ? `${repeatSelected} (تا تاریخ ${targetEndStr})` : "بدون تکرار"}`
    ].filter(Boolean).join("\n | ");
      ...
      {reminderMode === "judicial" ? (
        ...
        {isMoreInfoExpanded && (
          <div className="space-y-6 animate-in fade-in slide-in-from-top-1 duration-200">
            ...
            {/* Case Selector inside "More info" to maximize workspace capability */}
            <div className="flex items-center gap-4">
              <div className={`flex-1 relative border border-slate-200 ${themeFocusBorder} rounded-2xl h-15 bg-white transition-all shadow-sm`}>
                ...
                <select
                  ...
                  className="w-full h-full bg-transparent outline-none px-4 text-xs font-bold text-slate-700 text-right appearance-none cursor-pointer"
                >
                  <option value="">بدون ارتباط با پرونده خاص</option>
                  {cases.map((cs) => (
                    <option key={cs.id} value={cs.id}>
                      {cs.title} (کلاسه: {cs.archiveNumber || "ندارد"})
                    </option>
                  ))}
                </select>
