...
export default function BackupCenter({
  ...
  const handleSendEmailBackup = async () => {
    ...
    try {
      ...
      if (resJson.simulated) {
        ...
        link.href = url;
        link.download = `پشتیبان_کامل_و_مدارک_دفتر_وکالت_${new Date().toLocaleDateString("fa-IR").replace(/\//g, "-")}.json`;
        link.click();
  ...
  const handleFlashExport = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    // Attempting to suggest the requested sub-directory, though browser may strip it. 
    link.download = `پشتیبان گیری اتوماسیون هوشمند دفتر وکالت/Vakil_Backup_${new Date().toLocaleDateString("fa-IR").replace(/\//g, "-")}.json`;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-350">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-between space-y-6">
          <div className="space-y-4">
            ...
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] font-bold text-slate-700">
                <div className="bg-white border border-slate-150 p-2 rounded-xl text-center">
                  <span className="text-slate-450 block text-[9px]">موکلین:</span>
                  <span className="text-slate-850 font-mono text-xs">{toPersianDigits(clients.length)} مورد</span>
                </div>
                <div className="bg-white border border-slate-150 p-2 rounded-xl text-center">
                  <span className="text-slate-450 block text-[9px]">پرونده‌ها:</span>
                  <span className="text-slate-850 font-mono text-xs">{toPersianDigits(cases.length)} مورد</span>
                </div>
                <div className="bg-white border border-slate-150 p-2 rounded-xl text-center">
                  <span className="text-slate-450 block text-[9px]">یادداشت‌ها:</span>
                  <span className="text-slate-850 font-mono text-xs">{toPersianDigits(notes.length)} مورد</span>
                </div>
                <div className="bg-white border border-slate-150 p-2 rounded-xl text-center">
                  <span className="text-slate-450 block text-[9px]">جلسات/آلارم:</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4 border-t border-slate-100">
            <button
              ...
              className="py-2.5 bg-slate-950 hover:bg-black text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 select-none cursor-pointer"
            >
              <FolderDown className="w-4 h-4 text-amber-400" />
              دانلود نسخه پشتیبان (.json)
            </button>
            <label className="py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 select-none cursor-pointer text-center">
              <FolderUp className="w-4 h-4" />
              بازیابی فایل پشتیبان فلش
              <input
                type="file"
                accept=".json"
      ...
      {/* Connection and Transaction Activity Logs Console */}
      <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 space-y-3">
        <h3 className="text-xs font-black text-slate-700 flex items-center gap-2">
          <Activity className="w-4 h-4 text-slate-500" />
          کنسول زنده انتقال و امنیت داده‌ها
        </h3>
        <div className="bg-slate-950 text-slate-350 font-mono text-[10px] p-4 rounded-xl h-24 overflow-y-auto space-y-1.5 text-right leading-relaxed">
          {cloudLogs.length === 0 ? (
            <span className="text-slate-500">پورتال آماده پشتیبان‌گیری و دریافت فرامین است...</span>
          ) : (
            cloudLogs.map((log, index) => (
              <div key={index} className="border-b border-white/5 pb-1 last:border-b-0">
                {log}
              </div>
            ))
          )}
